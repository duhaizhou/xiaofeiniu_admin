<template>
    <div class="xfn-category-list">
      <el-breadcrumb>
        <el-breadcrumb-item to="/main">首页</el-breadcrumb-item>
        <el-breadcrumb-item>菜品类别管理</el-breadcrumb-item>
        <el-breadcrumb-item>类别列表</el-breadcrumb-item>
      </el-breadcrumb>
      <br>
      <el-button type="primary" @click="addCategory">添加新的菜品类别</el-button>
      <el-table :data="categoryList">
        <el-table-column label="编号" prop="cid"></el-table-column>
        <el-table-column label="名称" prop="cname"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="{row,$index}">
            <el-button type="success" @click="updateCategory(row,$index)" size="mini">修改</el-button>
            <el-button type="danger" @click="deleteCategory(row,$index)" size="mini">删除
            </el-button>
          </template>
          
        </el-table-column>
      </el-table>
    </div>
</template>
<script>
   export default{
     data(){
       return{
         url:'',
         categoryList:[]
       }
     },
     mounted() {
       this.url=this.$store.state.globalSettings.apiUrl+'/admin/category';
       this.$axios.get(this.url).then((res)=>{
         this.categoryList=res.data;
       }).catch((err)=>{
         console.log(err);
       })
     },
     methods: {
       deleteCategory(c,i){
         this.$confirm('此操作不可撤销，你确定要删除吗？','提示',{type:'warning'}).then(()=>{
           this.$axios.delete(this.url+c.cid).then((res)=>{
             if(res.data.code==200){//数据库中已经删除成功
               this.categoryList.splice(i,1);//模型数据中删除
               this.$message.success('菜品类别删除成功！');
             }else{
               this.$message.error('类别删除失败：'+res.data.msg)
             }
           }).catch((err)=>{
              console.log(err);
           })
         }).catch(()=>{})
         
         
       },
       updateCategory(c,i){
        this.$prompt('请输入您想修改的类别名：','提示',{inputValue:c.cname}).then(({value})=>{
          //得到用户输入的内容，调用数据API添加到数据库
          var url=this.$store.state.globalSettings.apiUrl+'/admin/category'
        })
       },
       addCategory(){
         this.$prompt('请输入新的菜品类别名：','提示',{type:'info'}).then(({value})=>{
           //获得用户输入的内容，调用数据API添加到数据库
           this.$axios.post(this.url,{cname:value}).then((res)=>{
             if(res.data.code==200){//数据库添加成功
               this.$message.success('新的类别添加成功！')
               //模型数据中添加新的类别
               this.categoryList.push({cid:res.data.cid,cname:value})
             }else{
               this.$message.error('新的类别添加失败：'+res.data.msg)
             }
           }).catch((err)=>{
             console.log(err)
           })
         }).catch(()=>{

         })
       }
     },
   } 
</script>
  